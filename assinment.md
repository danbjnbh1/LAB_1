...
1. Read, compile and run the `addresses.c` program (You can find the file in the course Moodle) (remember to use the `-m32` flag for the compiler).
2. Can you tell the location (stack, code, etc.) of each memory address?
3. What can you say about the numerical values? Do they obey a particular order?
4. Check long data size on your machine using `sizeof` operator. Is long integer data type enough for `dist` (address difference) variables?

### T1b - Arrays memory layout

In this task we will examine the memory layout of arrays.

Define four arrays of length 2 as shown below in the function main and print the memory address of each array cell.
...
3. Given the results, explain to the TA the memory layout of arrays.
...
1. Initialize the pointers `iarrayPtr` and `carrayPtr` to point to the first cell of the arrays `iarray` and `carray` respectively. 
2. Use the two pointers (`iarrayPtr`, `carrayPtr`) to print all the values of the two arrays.
3. Add an uninitialized pointer local variable `p`, and print its value (not the value it points to). What did you observe?
...
**Pointers to functions** - C allows declaring pointers to functions. The syntax is: `function_return_type (*pointer_name)(arguments_list);` for simple types of return value and arguments. You can read more about pointers to functions [here](https://www.cprogramming.com/tutorial/function-pointers.html).

The `base.c` file (You can find the file in the course Moodle) is the base file for task 2 - you should complete it as stated in the sub tasks.
...
Implement the `map` function that receives a pointer to a char (a pointer to a char array), an integer, and a pointer to a function. Map returns a new array (after allocating space for it), such that each value in the new array is the result of applying the function `f` to the corresponding character in the input array.
...
/* If c is a number between 0x20 and 0x7E, cprt prints the character of ASCII value c, 
   otherwise, the dot ('.') character. This is followed by a space character, and then 
   the value of c in a hexadecimal. Finally, a new line character is printed. 
...
**Note:** Array length is constant i.e. if the initial array is of length 5, then the new array that we receive with `my_get` function is of the same length.
...
Recall that typing 'Ctrl+D' on an empty line in the console simulates an EOF condition for stdin.

### T3b - Implementing the menu

In this task we will implement the menu. The menu must offer options for all your functions (Get String, Print Decimal (dprt), Print Char and Hex (xprt), Encrypt and Decrypt). See also the file Task 3 Example.
...
2. Define an array of `fun_desc` and initialize it (in the declaration, not as program code within a function) to the names and the pointers of the functions that you implemented in Task 2. The last `fun_desc` in the array should contain a null pointer name and a null pointer to function (the length of the array should not be kept explicitly after constructing it).
3. Display a menu (as a numbered list) of names (or descriptions) of the functions contained in the array. The menu should be printed by looping over the menu item names from the `fun_desc`, not by printing a string (or strings) that contain a copy of the name.
4. Display a prompt asking the user to choose a function by its number in the menu, read the number, and check if it is within bounds. The bound should be pre-computed only once, and before the loop where the prompt is printed. If the number is within bounds, "Within bounds" is printed, otherwise "Not within bounds" is printed and the program exits gracefully.
...
**Question:** Is it possible to call a function at an invalid address in your version of the program?
